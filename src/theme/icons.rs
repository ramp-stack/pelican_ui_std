...
pub struct Icon(IconName, &'static str, u32);
...
impl ComponentBuilder for Icon {
    fn build_children(&self, ctx: &mut ComponentContext, max_size: Vec2) -> Vec<Box<dyn Drawable>> {
        ...
        let (path, color, size) = self.unwrap();

        let size = size * 2; // scale factor broke

        let crate_root = env!("CARGO_MANIFEST_DIR");
        let path = PathBuf::from(crate_root).join(&format!("resources/icons/{}.svg", path));

        let mut svg_content = fs::read_to_string(&path).expect("Failed to read SVG file");
        svg_content = svg_content.replace("fill=\"white\"", &format!("fill=\"#{}\"", color));

        let parsed_file = nsvg::parse_str(&svg_content, nsvg::Units::Pixel, 96.0)
            .expect("Could not parse modified SVG");

        let rasterized = parsed_file.rasterize(8.0).expect("Could not rasterize image");
        let raw = RgbaImage::from_raw(rasterized.dimensions().0, rasterized.dimensions().1, rasterized.into_raw())
            .expect("Failed to create RgbaImage");

        Image(ShapeType::Rectangle(0, (size, size)), ctx.add_image(raw)).build_children(ctx, max_size)
    }
