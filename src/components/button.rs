...
use crate::{theme, Text, Row, Column, Stack};
...
impl Button {
    pub fn Primary(n: &'static str, s: Size, w: Width, ip: Option<&'static str>) -> Self {
        Self(n, s, w, ip, ButtonStyle::Primary, Align::Center)
    }
...
impl ComponentBuilder for Button {
    fn build_children(&self, ctx: &mut ComponentContext, max_size: Vec2) -> Vec<Box<dyn Drawable>> {
        let colors = theme::color::palette().button.colors_from(self.4, ButtonState::Default);
        ...
        let (text_size, height, icon_size, x_padding) = match self.1 {
            Size::Medium => (16, 32, 16, 24),
            Size::Large => (20, 48, 24, 48)
        };

        let mut content = Row!(3, Vec2::new(0, 0), Align::Center, false,
            // Image(ShapeType::Rectangle(icon_size, 8), image),
            Text::new(self.0, colors.label, text_size, font.clone())
        ).build(ctx, bound);

        let width = match self.2 {
            Width::Hug => content.size(ctx).x + x_padding,
