...
impl Events for AmountInputContent {
    fn on_event(&mut self, ctx: &mut Context, event: &mut dyn Event) -> bool {
        if let Some(KeyboardEvent{state: KeyboardState::Pressed, key}) = event.downcast_ref() {
            let font_size = ctx.get::<PelicanUI>().theme.fonts.size;
            let mut t = self.1.amount().text.replace(",", ""); // Clean commas from input
            let mut digit_count = t.chars().filter(|ch| *ch != '.' && *ch != '$').count();
            ...
            digit_count = t.chars().filter(|ch| *ch != '.' && *ch != '$').count();
